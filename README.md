...
- **Undo** / **Restart**. Five clears is today’s pack. A new local date starts a fresh 5-level pack and updates the streak (small chip on TAP TO PLAY). One hundred campaign levels; after 100 the campaign loops to 1. **Shop** lists locked mobile IAP prices (`com.obsidiancrow.tintdrop.<sku_id>`). F5 / editor does not charge. Entitlements save in `user://tint_drop.cfg`.
...
5. New levels go in `godot/scripts/main.gd` (`LEVELS`) and must pass `python3 tools/solve_levels.py`. Persist / daily pack: `python3 tools/test_progress.py`. Shop catalog: `python3 tools/test_shop.py`.
